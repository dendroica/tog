...
for (i in yrs) {
  ...
  lookup <- as.data.frame(t(sapply(split(tmp3, tmp3$STRATUM), function(z) {
    z[1, c("STRATUM", "STRATUM_AREA")]
  })))
  if (!all(as.numeric(colnames(tmp4)) - unlist(lookup[, "STRATUM"]) == 0)) {
    stop("strata misaligned")
  }
  ...
  out[, i - (min(yrs) - 1)] <-
    apply(
      tmp4, 1, function(z) {
        sum(z * unlist(lookup[, "STRATUM_AREA"]), na.rm = TRUE) #/ sum(unlist(lookup[, "STRATUM_AREA"]), na.rm = TRUE) I think in my case I take this out...
