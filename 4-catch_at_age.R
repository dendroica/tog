...
# Load ALKs
alks <- list.files(file.path(root, "output/tog/alk/filled/test"),
  full.names = TRUE
) |> map(read.csv)
# Folder contains...
...
# OUTPUTS#############
write.csv(caa_out, file.path(root, "output/tog/caa-alt.csv"))
write.csv(waa, file.path(root, "output/tog/waa-alt.csv"))
waa0 <- waa
waa0[is.na(waa0)] <- 0
