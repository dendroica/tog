...
ggplot(data = lifehistory, aes(x = Weight)) +
  geom_histogram() +
  facet_wrap(~Year) +
  xlab("Recreational Fish Weight (g)")

# We only need to use the recreational weights
weights_meansnj <- lifehistory %>%
  group_by(Year) %>%
  summarize(MeanWeight = mean(Weight, na.rm = TRUE), sdWeight = sd(Weight, na.rm = TRUE), .groups = "keep")

# Length-weight relationships for recreation harvested fish
ggplot(data = lifehistory, aes(x = Length, y = Weight, group = as.factor(Year), colour = as.factor(Year))) +
  geom_point()
...
discard_LF_propnj <- discard_LFnj %>% mutate(across(`2021`:`2024`, .fns = function(x) {
  x / sum(x, na.rm = TRUE)
}))
harvest_LF_propnj <- harvest_LFnj %>% mutate(across(`2021`:`2024`, .fns = function(x) {
  x / sum(x, na.rm = TRUE)
}))
